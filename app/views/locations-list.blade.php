    ...
    <a href="{{route('showLocation', $slug)}}" title="{{trans('locations.goTo',array('title'=>$title))}}">
      ...
      <div class="mainInfos">
        <div class="photo">
          ...
          <img itemprop="image" width="184" height="184" src="{{'/'.Config::get('var.images_dir').Config::get('var.users_dir').$location->building->user->id.'/'.Config::get('var.locations_dir').$location->id.'/'.Helpers::addBeforeExtension($location->accroche[0]->url, Config::get('var.img_medium'))}}" alt="{{trans('locations.photoOf',array('title'=>$title))}}">
          @else
