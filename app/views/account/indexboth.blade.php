<div class="home-location">
	...
	<div class="informations">
		...
		<p>{{trans('account.account_not_validate')}}</p>

		@endif
		@if(Auth::user()->email_comfirm == 0)
		<p>{{trans('account.email_not_comfirm')}}</p>
		@endif
		@if(Auth::user()->validate == 0 || Auth::user()->email_comfirm == 0)
		<div>
	...
	<div class="content">
				...
				<li class="location-account success">
					<a href="{{route('dashboard_location', array(Auth::user()->slug,$location->id))}}">	
						...
						<div class="infos">
							...
							@if(Helpers::isOk($building->address) && isset($location->translation))
		...
		<div class="invalidLocations">
			...
			<ul>
				...
				@foreach( $invalidLocations->building as $invalidLocation)
				...
				<li class="location-account invalid">
					<a href="{{route('dashboard_location', array(Auth::user()->slug,$location->id))}}">	
						...
						<div class="infos">
							...
							@else 
							<span class="building">{{trans('account.building_id',array('number'=>$building->id))}}</span>
							<span class="location">{{trans('account.location_id',array('number'=>$location->id,'type'=>isset($location->typeLocation->translation[0]->value) ? $location->typeLocation->translation[0]->value :''))}}</span>
							@endif
