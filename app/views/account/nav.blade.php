<div class="nav-account">
	<ul>
		...
		<span class="name">{{Auth::user()->name}} {{Auth::user()->first_name}}</span>
		...
		<li class="active">
			...
			<a  class="{{isset($personnalNotComplete) &&  Helpers::isOk($personnalNotComplete) && $personnalNotComplete->count > 0 ? 'hasInfo' : ''}}" href="{{route('account_personnal', Auth::user()->slug)}}" {{Helpers::isActive('account_personnal')}}>
		...
		<li>
			...
			<a href="{{route('account_params',Auth::user()->slug)}}" {{Helpers::isActive('account_params')}}>
